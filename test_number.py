...
def getNumberPlateData(path):
    ...
    if len(result) != 0:
        ...
        res = cv2.putText(gammaAdjImg, text=text, org=(coorX, coorY + 30), fontFace=font,
                          fontScale=fontscale, color=color, thickness=int(fontscale) + 1,
                          lineType=cv2.LINE_AA)  # Adding Text

        res = cv2.rectangle(gammaAdjImg, tuple(approx[0][0]), tuple(approx[2][0]), color, 3,
                            lineType=cv2.LINE_AA)  # Drawing Rectangle
